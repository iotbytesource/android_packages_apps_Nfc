...
#ifdef CONFIG_SNFC
static void nfc_jni_download_callback(void *pContext, NFCSTATUS status);
#endif
...
static int nfc_jni_download(struct nfc_jni_native_data *nat, uint8_t update)
{
    ...
    REENTRANCE_LOCK();
#ifdef CONFIG_SNFC
    status = secDnldNfc_StartThread((void *)&cb_data, nfc_jni_download_callback);
#else
...
static void nfc_jni_download_callback(void *pContext, NFCSTATUS status)
{
   ...
   LOG_CALLBACK("nfc_jni_download_callback", status);
...
#ifdef CONFIG_SNFC
   nat->discovery_cfg.NfcIP_Mode = (phNfc_ePassive106 | phNfc_ePassive212 | phNfc_ePassive424);  //initiator
#else
   nat->discovery_cfg.NfcIP_Mode = get_p2p_mode();  //initiator
#endif   
   nat->discovery_cfg.Duration = 300000; /* in ms */
